"""Visualization and lightweight analysis helpers used by demos."""
...
def summarize_label_histogram_alignment(graph_generator, graphs, n_compare=16):
    ...
    if getattr(graph_generator, "feasibility_estimator", None) is not None:
        ...
        matched_pairs = [
            (conditioning_hist, graph)
            for conditioning_hist, graph in zip(conditioning_histograms, decoded_slots)
...
def plot_similarity_distribution_with_iqr(sim_high, sim_low, target_high, target_low):
    ...
    plt.tight_layout()
    plt.show()
    print(f"{labels[0]} -> n={len(sim_high)}, median={med_high:.4f}, q1={q1_high:.4f}, q3={q3_high:.4f}")
    print(f"{labels[1]} -> n={len(sim_low)}, median={med_low:.4f}, q1={q1_low:.4f}, q3={q3_low:.4f}")
